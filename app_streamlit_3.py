...
# 你可保留一个常用默认路径，初次进入时显示（用户可自由切换）
DEFAULT_START_FOLDER = "/"

PROMPT_PATH = "Prompt.txt"
MODEL_NAME = "gpt-4-1106-preview"   # 如遇不可用，可换 "gpt-4o-mini"
...
def list_dropbox_folders(folder_path: str = "/"):
    """返回给定路径下的子文件夹列表：[(name, path_lower), ...]"""
    try:
        dbx = get_dbx()
        ...
        st.error("Dropbox 认证失败，请检查 refresh token 与应用权限。")
        st.stop()
...
def list_dropbox_files(folder_path: str):
    """仅列出当前文件夹中的 .pdf/.docx 文件（不递归）。返回 [(name, path_lower), ...]"""
    try:
        dbx = get_dbx()
        result = dbx.files_list_folder(folder_path)
        files = []
...
def download_dropbox_files(files, selected_names):
    """根据 multiselect 选择的文件名下载，返回 [(name, content_bytes), ...]"""
    try:
        dbx = get_dbx()
...
def write_output_to_word(results):
    """results: [(filename, output_text), ...] -> BytesIO"""
    doc = DocWriter()
...
go_col1, go_col2 = st.columns([1, 4])
with go_col1:
...
st.caption("Tip: You can type a path in 'Current path' or pick from the list and click 'Open selected'.")

# Step 1: 在选定文件夹中选择要处理的文件
...
elif st.button("Start Processing") and not selected_files:
